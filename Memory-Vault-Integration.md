...
> *This is the detailed integration guide for Memory Vault. For an overview of all integrations, see [INTEGRATION.md](INTEGRATION.md).*
...
```python
# In your IntentLog persistence logic
...
| IntentLog Type | Recommended Vault Level | Justification Requirement | Cooldown |
|----------------|------------------------|--------------------------|----------|
...
1. Add `intent_ref` to your IntentLog schema
2. Route high-value entries through `store_critical_intent()`
3. On agent reflection/goal recall:
   - Check if intent has a vault `memory_id`
   - If yes → use `recall_critical_intent()` with justification
4. Use Vault search for powerful reflection:

```python
from memory_vault.db import search_memories_metadata
